...
class Main:
    ...
    def readinfo(self,mode):
        if mode == 'min':
            self.config['STATE']['cpu_usage'] = str(psutil.cpu_percent(self.timer))
        ...
        if mode == '-' :
            for i in range(self.nbcpu - int(self.config['CONFIG']['min_active_cores']) + 1):
                if self.config['CORES'][str(self.nbcpu - i)] == str(1):
                    ...
                    print("core " + str(self.nbcpu -i+1) + " offline")
                    break
        elif mode == '+' :
            for i in range(self.nbcpu+1):
    ...
    def main(self):
        ...
        while True:
            ...
            if float(self.config['STATE']['cpu_usage']) < int(self.config['CONFIG']['low_usage']):
                if str(self.config['CORES'][str(int(self.config['CONFIG']['min_active_cores']))]) == str(0):
                    self.timer = 0.4
                else:
                    self.timer = 0.05
                    self.managecpucores('-')
